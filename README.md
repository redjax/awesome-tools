...
- [Scoop CLI](https://scoop.sh) - A package manager with a bias towards free, open-source CLI tools. The community can add additional packages with [the "extra" bucket](https://github.com/ScoopInstaller/Extras). All programs are installed to a path under your user directory, keeping everything in 1 place (as opposed to the buckshot of a regular Windows program installation) and making uninstallation simple.
- [Chocolatey CLI](https://chocolatey.org/install) - The grandaddy of 3rd-party Windows package managers. Very large [package repository](https://community.chocolatey.org/packages), but a little more complex & difficult. Has [an enterprise offering](https://chocolatey.org/products/chocolatey-for-business), where businesses can maintain their own private package repository, and can be used as a [private/personal package repository](https://blog.chocolatey.org/2016/01/host-your-own-server/). Highly scriptable, but being an old package manager, it can sometimes be clunky. Old & stable is a feature for many, & worth learning the quirks.
...
- [Flow Launcher](https://github.com/Flow-Launcher/Flow.Launcher) - An `ALT+SPACE` launcher, like Mac's Spotlight. Configurable with plugins & follows [Microsoft's Fluent design language](https://fluent2.microsoft.design) so it feels like a more 'native' experience. Quickly run shell commands, launch programs, do calculations, spawn web searches, [& more](https://github.com/Flow-Launcher/Flow.Launcher?tab=readme-ov-file#-features).
